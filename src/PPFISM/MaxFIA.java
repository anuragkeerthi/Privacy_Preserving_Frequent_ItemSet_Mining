...
public class MaxFIA 
{
	...
	for (Integer current :sensitiveTransactions ) 
      ...
      //  System.out.println(sortedSensitiveTransactions.subList(0, num)); // Number of transactions to be cleared based on input PSI
        ...
        ArrayList<ArrayList<Integer>> maxSupportTransactions = new ArrayList<ArrayList<Integer>>() ;
       ...
       int max , maxID;
        for(int i = 0; i < sensitivePatterns.size() ; i++)
        {
            ...
            for(int j = 0; j < sensitivePatterns.get(i).size() ; j++ )
            {
                ...
                {
                    if(sensitivePatterns.get(i).get(j) == supportCount.get(k).get(0) && supportCount.get(k).get(1) > max)
                    {
                        max = supportCount.get(k).get(1);
                        maxID = supportCount.get(k).get(0);
            ...
            temp.add(maxID);
             maxSupportTransactions.add(temp);
        }
        
         //System.out.println(maxSupportTransactions);
        
        
        
        for(int k = 0; k<maxSupportTransactions.size();k++)
        {
        for( int i = 0; i<inputSensitiveTransactions.size();i++)
        {
            itemsTransactionsClone.get((inputSensitiveTransactions.get(i)) - 1).removeAll(maxSupportTransactions.get(k));
        }
        }
    ...
    public  void ArrayListToTableWithCopy(ArrayList<ArrayList<Integer>> input , String tableName)
     {
        ...
        String drp2="drop table MaxFIAOutput";
         ...
         try
               {
                ...
                String q2="create table MaxFIAOutput( Tid integer"+createString+")"; //Cloning the table for future use
          ...
          try
          {
              ...
              String cloneTable="insert MaxFIAOutput select * from "+tableName;
